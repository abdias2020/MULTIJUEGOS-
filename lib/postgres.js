...
export async function getSubbotConfig(botId) {
  try {
    ...
    if (res.rows.length > 0) {
      ...
      // 🔥 FORZAR MODO PÚBLICO SI ESTÁ EN PRIVADO
      if (config.mode === 'private') {
        console.log('⚠️ Bot configurado en modo privado, cambiando a público...');
        await db.query("UPDATE subbots SET mode = 'public' WHERE id = $1", [cleanId]);
        config.mode = 'public';
      }
    ...
    // Si no existe en DB, crear entrada con modo público
    console.log('📝 Creando configuración inicial para bot:', cleanId);
    await db.query(
      "INSERT INTO subbots (id, mode, prefix, owners) VALUES ($1, $2, $3, $4)",
      [cleanId, 'public', ['/', '.', '#', '!'], ['51981557640@s.whatsapp.net', '51970454739@s.whatsapp.net']]
    );
    ...
    return {
      ...
      anti_call: false,
      owners: [
        '51981557640@s.whatsapp.net',
        '51970454739@s.whatsapp.net'
      ],
      name: null,
      logo_url: null,
      privacy: null,
