...
let handler = async (m, { conn, text, args, usedPrefix, command }) => {
  ...
  if (command == 'nserie' || command == 'myns' || command == 'sn') {
    ...
    await conn.fakeReply(m.chat, sn, '0@s.whatsapp.net', `⬇️ ᴇsᴛᴇ ᴇs sᴜ ɴᴜᴍᴇʀᴏ ᴅᴇ sᴇʀɪᴇ ⬇️`, 'status@broadcast');
  }
  ...
  if (command == 'unreg') {
    ...
    await conn.fakeReply(m.chat, `😢 Ya no estás registrado`, '0@s.whatsapp.net', `ᴿᵉᵍᶦˢᵗʳᵒ ᵉˡᶦᵐᶦⁿᵃᵈᵒ`, 'status@broadcast');
  }
