const handler = async (m, { conn }) => {
  const now = Date.now();
  const cooldown = 60_000; // ⏱️ 1 minuto

  // 💎 Más XP y mayor probabilidad de premios altos
  // Multiplica el rango a 12,000 y sesga hacia los valores grandes
  const hasil = Math.floor(Math.pow(Math.random(), 0.4) * 12000) + 1000; // mínimo 1000 XP, máximo 12000 XP aprox.
  ...
  if (restante > 0)
    return m.reply(`⏳ 𝐄𝐬𝐩𝐞𝐫𝐚 *${msToTime(restante)}* 𝐩𝐚𝐫𝐚 𝐯𝐨𝐥𝐯𝐞𝐫 𝐚 𝐦𝐢𝐧𝐚𝐫`);
